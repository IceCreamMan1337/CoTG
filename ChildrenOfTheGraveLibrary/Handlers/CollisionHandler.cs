...
namespace ChildrenOfTheGrave.ChildrenOfTheGraveServer.Handlers
{
    /// <summary>
    ...
    internal class CollisionHandler
    {
        ...
        private readonly List<(AttackableUnit, List<NavigationGridCell>)> _previousClosedCells = new();
        ...
        private static bool IsCollisionAffected(GameObject obj)
        {
            ...
            return !(obj.IsToRemove() || obj is LevelProp || obj is Particle || obj is ObjBuilding || obj is BaseTurret || obj is ObjAIBase { Stats.IsDead: true } || obj is Champion { Stats.IsDead: true });
        }
        ...
        private void ReopenClosedCells()
        {
            foreach (var (_, closedCells) in _previousClosedCells)
            {
        ...
        public void UpdateCollision(GameObject obj)
        {
            ...
            {
                if (!Game.Map.NavigationGrid.IsWalkable(obj.Position))
                {
                    obj.OnCollision(null, CollisionTypeOurs.TERRAIN);
                }

                if (obj.PathfindingRadius > 0 && obj is AttackableUnit unit && !unit.IsWalking())
                {
                    var closedCells = NavigationGrid.CloseCellsWithUnit(unit);
                    _previousClosedCells.Add((unit, closedCells));
                ...
                foreach (var obj2 in nearest)
                {
                    ...
                    if (obj != obj2 && !obj2.IsToRemove() && obj.IsCollidingWith(obj2))
                    {
                        ...
                        if (obj.IsPathCollidingWith(obj2))
                        {
                            collisionType = CollisionTypeOurs.BOTH_RADII;
                        }
                        obj.OnCollision(obj2, collisionType);
