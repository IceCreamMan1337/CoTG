...
using PARType = ChildrenOfTheGraveEnumNetwork.Enums.PrimaryAbilityResourceType;
...
public static partial class Functions
{
    ...
    {
        //so this is an "approx" fix , is not perfecxt but it does the coffee 
        float goldPerSecond = delta / 10000f;

        if (target is IGoldOwner goldOwner)
        {
            goldOwner.GoldOwner.AddGold(goldPerSecond, false);
        }
