...
internal static class LuaScriptEngine
{
    ...
    private static readonly Dictionary<string, ScriptCacheInfo> Cache = new();

    private static readonly Dictionary<string, BBCacheEntry> BBCache = new();
    ...
    static LuaScriptEngine()
    {
        ...
        FindScriptFiles(Path.Combine(ContentManager.DataPath, "DATA/Scripts"));
        FindScriptFiles(Path.Combine(ContentManager.DataPath, "DATA/Spells"));
        if (Game.Config.DeployFolder != null && Directory.Exists(Game.Config.DeployFolder))
        {
            FindScriptFiles(Path.Combine(Game.Config.DeployFolder, "DATA"));
        }
        ...
        var globalsTable = Script.Globals;

        foreach (var keyValuePair in globalsTable.Pairs)
        {
    ...
    [MoonSharpModuleMethod]
    public static DynValue LoadScriptIntoScript(ScriptExecutionContext executionContext, CallbackArguments args)
    {
        return DoLua(nameof(LoadScriptIntoScript), executionContext, args);
    }
    ...
    [MoonSharpModuleMethod]
    public static DynValue LoadScriptDefaultNeutralMinionValues(ScriptExecutionContext executionContext, CallbackArguments args)
    {
        return DoLua(nameof(LoadScriptIntoScript), executionContext, args);
    }
    ...
    private static void FindScriptFiles(string folder)
    {
        ...
        {
            // Check that the extension is .lua or .preload regardless of case
            if (filepath.EndsWith(".lua", StringComparison.OrdinalIgnoreCase) || filepath.EndsWith(".preload", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    _logger.Error($"File name collision of {existing} and {filepath}");
                }

                // Stockage du chemin du fichier avec un nom en minuscules
    ...
    internal static DynValue DoScript(string luaName, Table globals)
    {
        ...
        ScriptCacheInfo? s;
        if (Cache.TryGetValue(luaName, out s))
        {
            ...
            if (!found) _logger.Error($"{luaName} has no cache");
        }
        else
        {
            Cache[luaName] = s = new ScriptCacheInfo();

            if (luaName.EndsWith(_BB_LUA))
            {
                var preloadName = luaName.Substring(0, luaName.Length - _BB_LUA.Length) + _PRELOAD_BB_LUA;
                if (Files.TryGetValue(preloadName, out var preloadPath))
                {
