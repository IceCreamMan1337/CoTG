...
using F = ChildrenOfTheGrave.ChildrenOfTheGraveServer.Scripting.Lua.Functions;
//using FCS = ChildrenOfTheGrave.ChildrenOfTheGraveServer.Scripting.CSharp.Converted.Functions_CS;
using AFM = ChildrenOfTheGrave.ChildrenOfTheGraveServer.API.ApiFunctionManager;
...
public static class Functions_BBB_and_CS
{
   ...
   )
    {
        ...
        return AFM.AddPet(
            petOwner, originSpell,
            pos.ToVector2(), name, skin, buff, duration,
    ...
    )
    {
        ...
        //   
        //   
        return AFM.AddPetClone(
            petOwner, originSpell,
            unitToClone as ObjAIBase, //TODO: Either allow any unit to be cloned, or change the argument type
            pos.ToVector2(), buff, duration,
            cloneInventory: true,
            showMinimapIcon,
            disallowPlayerControl: false,
            doFade: false
    ...
    )
    {
      ...
      originSpell
        ...
        /*
        if (target.GetBuffWithName(buffScript.Name).BuffScript.MetaData.SpellToggleSlot != null && originSpell != null)
        {
            Console.WriteLine(originSpell);
            Console.WriteLine(originSpell.Name);

            if (target.GetBuffWithName(buffScript.Name).BuffScript.MetaData.SpellToggleSlot == 1)
            {
                Console.WriteLine("test toggle spell");
                Game.PacketNotifier.NotifyChangeSlotSpellData((int)target.NetId,
                    ...
                    false,
                    TargetingType.Self,
                    target.GetBuffWithName(buffScript.Name).BuffScript.MetaData.BuffName);
            }
        } 
        */
    }
    ...
    {
        int buffCount = F.SpellBuffCount(target, buffName, caster as ObjAIBase);

        // Console.WriteLine(buffCount);

        return buffCount >= 0 ? buffCount : 0;
    }
