...
using ChildrenOfTheGraveLibrary.Content;
...
using E = ChildrenOfTheGraveEnumNetwork.Extensions;
...
public static partial class Functions
{
    ...
    public static List<RestrictedZone> RestrictedZones { get; } = new List<RestrictedZone>();
    ...
    [BBFunc(Dest = "Position")]
    public static Vector3 GetPointByUnitFacingOffsetposition(AttackableUnit unit, Vector3 spellposition, float distance, float offsetAngle)
    {
        var unitPos = unit.Position3D;


        var dir = (spellposition - unitPos).ToVector2().Normalized();
    ...
    )
    {
        ...
        m.ChangeGoldRedirectTarget(goldRedirectTarget);
        m.SetStatus(StatusFlags.Stunned, stunned);
        m.SetStatus(StatusFlags.Rooted, rooted);
        m.SetStatus(StatusFlags.Silenced, silenced);
        m.SetStatus(StatusFlags.Invulnerable, invulnerable);
        m.SetStatus(StatusFlags.MagicImmune, magicImmune);
        if (placemarker)
        {
    ...
    [BBFunc]
    //This method sets the cooldown time of a spell in a given spell slot for a given AI owner. The parameters include the AI owner, the spell slot, the spellbook type
    //, the slot type (defaults to SpellSlots), and the source of the cooldown time (defaults to 0). The method first converts the spell slot to the appropriate API slot
    //, then sets the cooldown of the spell in that slot to the given source.
    ...
    {
        // Check for ResistantSkin immunity (Baron Nashor and other immune units)
        if (unit.HasBuff("ResistantSkin") || unit.HasBuff("ResistantSkinDragon"))
        {
            return; // Immune units cannot be moved by displacement abilities
        }

        if (IsDestinationRestricted(target.ToVector2(), unit as ObjAIBase))
        {
            target = RestrictedAreaPos(target, unit as ObjAIBase);
        }
        unit._ForceMovementType = movementType;
        unit.DashToLocation(
            target.ToVector2(),
            speed, //TODO: Check if conversions are needed.
            gravity * 0.5f,//* 0.5f, //TODO: Check constant.
    ...
    )
    {
        ...
        {// i try do some test with flash 
            if (effectName == "summoner_flash.troy" || effectName == "summoner_flashback.troy")
                flags = FXFlags.BindDirection; //Checkifspellwitheffectnameexist(effectName)
        }
        ;


        if (effectName.Contains("beam") || effectName.Contains("beam"))
        {
            var p1 = new Particle(effectName,
                caster,
                pos.ToVector2(),
                bindObject,
                boneName ?? "",
                ...
                1.0f,
                default,
                particleEnemyName: effectNameForOtherTeam ?? null,
                false
                );
            effectID = p1;
            effectID2 = null;
    ...
    /// <summary>
    /// we does this for get spellinfo , is in them who have flags 
    /// </summary>
    /// <param name="_name"></param>
    /// <returns></returns>

    /// <summary>
    /// used for convert flags to bin
    /// </summary>
    ...
    {
        if (attacker == target && target.Model.Contains("Twitch"))
            return;
        ...
        //hack : in league, they use some parent method for get name of spell/buff etc... 
        //actually i will use c# fucntion for "emulate it" 
        StackTrace stackTrace = new();
        StackFrame frame = stackTrace.GetFrame(1); // get frame of parent ( 0 is actual frame ) 
    ...
    )
    {
        ...
        if (location != targetPosition)
        {
            ...
            target.FaceDirection(direction, isInstant: true);
        }
    }
    ...
    )
    {
        ...
        {
            if ((owner as Champion).Model == "Katarina")
            {
                if (owner.Spells[slot].CurrentCooldown <= 1)
                {
                    return owner.Spells[slot].Cooldown;
                }
            }
        }
    ...
    {
        int slot = ConvertAPISlot(spellSlot, slotType, spellbookType);
        return owner.Spells[slot]?.Level ?? 0;
    }
    ...
    )
    {
        ...
        if (slot < 0 || slot >= target.Spells.Count())
        {
            return;
        }

        var spell = target.Spells[slot];
        if (spell == null)
        {
            return;
        }
        ...
        if (target is Champion champion)
        {
            ChangeSlotSpellDataNotifier(
                ...
                target,
                slot,
                ChangeSlotSpellDataType.SpellName,
                newName: spell.Name ?? string.Empty
            );

        }
        if (state == false)
        {
            SetSlotSpellCooldownTime(target, spellSlot, spellbookType, slotType, target.Spells[slot].CurrentCooldown);
        }
    ...
    )
    {
        owner.SetAnimStates(new Dictionary<string, string>{
            { toOverrideAnim.ToLower(), overrideAnim }
        });
    }
    ...
    )
    {
        ...
        {
            Champion c => c.Experience?.Level ?? 1,
    ...
    {
        if (Game.Config.EnableLogAndConsole)
        {
            _logger.Info($"{owner.Model} {nameof(Say)} {toSay} {src}");
        }
    }
    ...
    )
    {
        ...
        (owner as ObjAIBase)!.SetAutoAttackSpell(slot, cancelAttack, autoAttackSpellLevel);
    }
    ...
    {
        _logger.Debug($"{owner.Model} {nameof(DebugSay)} {toSay} {src}");
    }
    ...
    {
        if (IsDestinationRestricted(castPosition.ToVector2(), owner as ObjAIBase))
        {
            castPosition = RestrictedAreaPos(castPosition, owner as ObjAIBase);
        }
    ...
    public static void PopCharacterData(
        AttackableUnit target,
        int ID,
        bool overrideSpells = false
    )
    {
        (target as ObjAIBase)?.PopCharacterData(ID);
    }

    //hack because nidalee is special 
    public static int PushCharacterData2(
       string skinName,
       AttackableUnit target,
       bool overrideSpells
   )
    {
        return (target as ObjAIBase)?.PushCharacterData2(skinName, overrideSpells) ?? 0;
    }


    public static int PushCharacterData3(
       string skinName,
       AttackableUnit target,
       bool overrideSpells
   )
    {
        return (target as ObjAIBase)?.PushCharacterData3(skinName, overrideSpells) ?? 0;
    }

    [BBFunc]
    public static void PopCharacterData2(
        AttackableUnit target,
        int ID,
        bool overrideSpells = false
    )
    {
        (target as ObjAIBase)?.PopCharacterData2(ID, overrideSpells);
    }
    ...
    )
    {
        ...
        var obj = Game.Map.CollisionHandler.GetNearestObjects(target);
        foreach (var obj2 in obj)
        {

            if (target != obj2 && target.IsCollidingWith(obj2))
            {
                var collisionType = CollisionTypeOurs.REGULAR_RADIUS;
                if (target.IsPathCollidingWith(obj2))
                {
                    collisionType = CollisionTypeOurs.BOTH_RADII;
                }
                target.OnCollision(obj2, collisionType);
    ...
    {
        var slot = ConvertAPISlot(slotNumber, slotType);
        ((ObjAIBase)target).Spells[slot].TargetingType = targetType;


        //test : 
        ChangeSlotSpellDataNotifier
                    (
        (target as Champion).ClientId, target as ObjAIBase, slot,
                        ChangeSlotSpellDataType.TargetingType, targetingType: targetType
                    );
    ...
    {
        S2C_ChangePARColorOverrideNotifier(owner as Champion, (byte)colorR, (byte)colorG, (byte)colorB, (byte)colorA, (byte)fadeR, (byte)fadeG, (byte)fadeB, (byte)fadeA);
    }
    ...
    {
        Vector2 unitPos = unit.Position3D.ToVector2();
        Vector2 targetPos = offsetPoint.ToVector2();
        Vector2 desiredDirection = targetPos - unitPos;
        Vector2 unitFacingDirection = unit.Direction.ToVector2().Normalized();

        // Calculate the angle between the unit's facing direction and the desired direction
        float desiredAngle = desiredDirection.Angle();
        float facingAngle = unitFacingDirection.Angle();

        // Return the relative angle offset
        float relativeAngle = desiredAngle - facingAngle;
    ...
    )
    {
        ...
        TintNotify(specificToTeam, true, fadeTime, maxWeight, color);
    }
    ...
    )
    {
        ...
        TintNotify(specificToTeam, false, fadeTime, 0, color);
    }
    ...
    {
        //check if it's suffisent 
        if (target is Champion)
        {
            if ((target as Champion).Stats.IsDead)
            {
                (target as Champion).Respawn();
            }
        }
    ...
    {
        _logger.Warn($"{nameof(Alert)} {toAlert} {src}");
    }
    ...
    )
    {
        ...
        spellSlotOwner.LevelUpSpell((byte)slot, false);
    }
    ...
    {
        var slot = ConvertAPISlot(spellSlot, slotType);
        return (spellSlotOwner as ObjAIBase).Spells[slot].ManaCostIncreaseMultiplicative;
    ...
    {
        _logger.Warn($"{nameof(_ALERT)} {smth}");
    }
    ...
    {
        //distancesquarred seem more logical 
    ...
    {
        //     DebugSay(default, "");
        var inhib = Inhibitor.GetInhibitor((TeamId)team, (Lane)lane);
        //  inhib.SetStatus(StatusFlags.Invulnerable, false);
        //  inhib.SetStatus(StatusFlags.Targetable, true);
        //  Game.PacketNotifier126.NotifyOnReplication(inhib, -1);
        return inhib;
    }
    ...
    public static void DisableHUDForEndOfGame()
    {
        ...
        DisableHUDForEndOfGameNotify();
        DebugSay(default, "test");
    }
    ...
    public static void MoveCameraFromCurrentPositionToPoint(Champion champion, Vector3? luaPosition, float travelTime, bool unk)
    {
        ...
        MoveCameraToPointNotify(info, Vector3.Zero, luaPosition ?? Vector3.Zero, travelTime, true, unk /*check*/);
    }
    ...
    public static void SetGreyscaleEnabledWhenDead(Champion champion, bool enabled)
    {
        ...
        if (info is not null)
        {
            SetGreyscaleEnabledWhenDeadNotify(info, enabled);
        }
    }
    ...
    /// 
    /// </summary>
    /// <param name="function"> NeutralMinionCamps[r].Timer, </param>
    /// <param name="timerType"> NeutralMinionCamps[r].TimerType, </param>
    /// <param name="delay"> NeutralMinionCamps[r].GroupsRespawnTime, </param>
    /// <param name="spawnDuration"> 0,</param>
    /// <param name="repeat">false </param>
    ...
    public static void CreateNeutralCamp_CS(CampData neutralCamp, int groupNumber)
    {
        ...
        camp.SetSpawnEndTime(Game.Time.GameTime + neutralCamp.GroupDelaySpawnTime + DEFAULT_CAMP_DELAY_SPAWN_TIME); //maybe we could ignore the const in this case?
        NeutralCampManager.CreateMinionCamp(neutralCamp.Positions[0], neutralCamp.MinimapIcon, groupNumber, neutralCamp.GroupBuffSide, neutralCamp.RevealEvent, (int)HashString(neutralCamp.TimerType));
    ...
    public static void SpawnNeutralMinion_CS(CampData neutralCamp, int groupNumber, int groupIndex, int nameIndex)
    {
    ...
    {
        foreach (Champion ch in Game.ObjectManager.GetChampionsInRange(obj.Position3D.ToVector2(), radius, true))
        {
            if (exp < 0)
            {
                throw new Exception("be aware exp was negative ");
            }
            ch.Experience.AddEXP(exp + obj.Stats.ExpGivenOnDeath.Total);
        }
    }
    ...
    public static void AIScriptSpellBuffAdd(AttackableUnit target, ObjAIBase caster, string buffName, int buffType,
        float duration)
    {
        ApiFunctionManager.AddBuff(buffName, duration, 1, null, caster, target);
    }

    public static void AIScriptSpellBuffRemove(AttackableUnit target, string buffName)
    {
        ApiFunctionManager.RemoveBuff(target, buffName);
    }
    ...
    public static void AddRestrictedZone(float top, float left, float width, float height, Vector2 position, float radius, Vector3 position3D)
    {
        RestrictedZone zone = new(top, left, width, height);
        RestrictedZones.Add(zone);
        zone.AddZone(position, radius, position3D);
    }

    public static void RemoveRestrictedZones()
    {
    ...
    public static bool IsDestinationRestricted(Vector2 castPos, ObjAIBase owner = null)
    {
        if (RestrictedZones != null || RestrictedZones.Count > 0)
        {
            foreach (var zone in RestrictedZones)
            {
                if (zone != null && !zone.IsInsideRestrictedZone(castPos, 200))
                {
                    return true;
                }
            }
    ...
    public static Vector3 RestrictedAreaPos(Vector3 castPos, ObjAIBase owner = null)
    {
        Vector3 finalPos = castPos;
        ...
        foreach (var zone in RestrictedZones)
        {
            if (zone != null)
            {
                float distBetweenOwnerAndRZ = DistanceBetweenPoints(zone.Position, ownerPos);

                if (distBetweenOwnerAndRZ < minDistance)
                {
                    minDistance = distBetweenOwnerAndRZ;

                    // Calculer finalPos seulement si c'est la zone la plus proche jusqu' prsent
                    float finalDist = 400 - distBetweenOwnerAndRZ;
                    finalPos = GetPointByUnitFacingOffset(owner, finalDist, 0);
                }
            }
        }
    ...
    public static void SetGameScoreCS(TeamId team, int score = 0)
    {
