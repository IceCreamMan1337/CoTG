...
public class BBBuffScript : BBScriptInstance, IBuffScriptInternal
{
    private readonly List<Particle> EffectsCreatedOnActivation = new();
    ...
    public BBBuffScript(BBScriptCtrReqArgs baseArgs, GameObject baseAttacker) : base(baseArgs)
    {
        ...
        MetaData = _bb.CacheEntry.GetMetadata<BuffScriptMetadataUnmutable>();
        ...
        for (var i = 0; i < m.AutoBuffActivateEffect.Length; i++)
        {
            ...
            if (!string.IsNullOrEmpty(effect))
            {
                var bone = (
                    ...
                    m.AutoBuffActivateAttachBoneName.Length > i ? m.AutoBuffActivateAttachBoneName[i] : null
                ) ?? "";
