...
namespace ChildrenOfTheGraveLibrary.Scripting.Lua.Scripts;

[MoonSharpModule]
internal class LuaAIScript : CAIScript, IAIScriptInternal
{
    private static readonly ILog Logger = LoggerProvider.GetLogger();
    private readonly Table _aiScriptGlobals;
    private readonly Dictionary<string, RunningTimer> _timersLua = new();

    private int _leashCounter;
    private Vector3 _leashedOrientation;
    private Vector2 _leashedPos;

    private readonly Dictionary<string, Dictionary<int, MethodInfo>> _methodsCache = new();
   ...
   function TimerDistanceScan()
    ...
    private void RegisterAllMethods()
    {
                     ...
                     .GroupBy(m => m.Name))
        {
            ...
            foreach (var methodInfo in methodGroup)
            {
                ...
                {
                    _methodsCache.Add(methodGroup.Key, new Dictionary<int, MethodInfo>());
                    _methodsCache[methodGroup.Key].Add(methodInfo.GetParameters().Length, methodInfo);
                }
            }
    ...
    {
        //oncallforhelp not totally implemented
        if (functionName != "OnCallForHelp")
            try
            {
