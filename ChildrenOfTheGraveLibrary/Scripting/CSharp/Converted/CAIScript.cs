...
using ChildrenOfTheGraveEnumNetwork.Domain;
...
using ChildrenOfTheGraveLibrary.Scripting.Lua;
using System.Linq;
...
public partial class CAIScript : IAIScriptInternal
{
    private readonly Dictionary<AttackableUnit, float> _ignoreTargetList = new();
    ...
    public static List<Action> TaskAction = new();
    ...
    protected void ClearValidTargets()
    {
        Me.TargetUnit = null;
    }
    ...
    private Vector2 GetLaneMinionWaypoint(LaneMinion minion)
    {
        bool WaypointReached()
        {
            ...
            if (minion.PathingWaypoints.Count() < _currentWaypointIndex + 1)
            {
                //hack minion sometimes finish their path , and crash , so we will just say it's their position 
                minion.PathingWaypoints.Add(minion.Position);
            }
            ...
            if (Game.Map.Id == 8)
            {
                if (Vector2.Distance(minion.Position, currentWaypoint) < 150.0f)
                {
                    return true;
                }
            }
    ...
    /*
    bad method for restrict movement 
    ...
    */
    /*in nseai TimeBetweenRepathsInMS = 250 */

    private float _lasttickupdatemanager;

    private float tickupdatemovement = 500f;
    ...
    protected void SetStateAndMoveToForwardNav(AIState state)
    {
        ...
        if (Me is LaneMinion minion)
        {
            ...
            //but this one , brake the lua clock 
            // float time = (Game.Time.GameTime / 5f);

            // if ((time - (Me.lasttimegetupdated) >= tickupdatemovement) || Me.lasttimegetupdated == 0.0f)
            //{
            Me.IssueOrDelayOrder(OrderType.AttackTo, null, GetLaneMinionWaypoint(minion), fromAiScript: true);
            //  Me.lasttimegetupdated = time;
            // }
        }
    }
    ...
    protected Vector2 MakeFleePoint()
    {
        ...
        var randomPos = Functions.GetRandomPointInAreaPosition(Me.Position3D, FEAR_WANDER_DISTANCE, 100);
    ...
    //todo: implement ( minion odin ) 
    protected Object FindUsableObjectInAcR()
    {
        // TargetPos = pos;
        return default;
    }
    ...
    //todo : implement  ( minion odin ) 
    protected void UseTarget(Object thing)
    {

    }
    //todo : implement  ( minion odin ) 
    protected bool IsObjectUsable(Object thing)
    {
        return default;
    }
    //todo : implement  ( minion reverse possibly odin too  ) 
    protected bool IsObjectUseable(Object thing)
    {
        return default;
    }
    //todo : implement  ( minion reverse possibly odin too ) 
    protected bool SetStateAndMoveToBackwardNav(Object thing)
    {
        return default;
    }

    //todo : implement  ( minion reverse possibly odin too ) 
    protected Object FindUseableObjectInAcR()
    {
        // TargetPos = pos;
        return default;
    }

    //todo : implement  ( petAI  ) 
    protected bool IsHardState()
    {
        // TargetPos = pos;
        return default;
    }
    ...
    //todo : implement  ( petAI  ) 
    protected bool IsOrderType(Object thing)
    {
        // TargetPos = pos;
        return default;
    }
    ...
    //todo : implement  ( petAI  ) 
    protected bool HandleIdleAttackStates(Object thing)
    {
        // TargetPos = pos;
        return default;
    }
    ...
    //todo : implement  ( petAI  ) 
    protected bool IsFarFromOwner()
    {
        // TargetPos = pos;
        return default;
    }
    //todo : implement  ( petAI  ) 
    protected bool IsHoldPositionState()
    {
        // TargetPos = pos;
        return default;
    }
    //todo : implement  ( petAI  ) 
    protected bool HandleTargetAttackStates(Object thing)
    {
        // TargetPos = pos;
        return default;
    }
    //todo : implement  ( petAI  ) 
    protected void HandleStateTransitions(Object thing)
    {
        // TargetPos = pos;
    ...
    //todo : implement  ( petAI  ) 
    protected void HandleEnemyFinding()
    {
        // TargetPos = pos;
