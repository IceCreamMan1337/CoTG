...
namespace ChildrenOfTheGraveEnumNetwork
{
    ...
    /// however, ChildrenOfTheGrave has its origin at the bottom left corner of the map (behind blue fountain), so a conversion is needed.
    /// </summary>
    public class MovementVector
    {
        ...
        public static Vector2 ToCenteredScaledCoordinates(Vector2 coords, NavigationGrid navGrid)
        {
            return new Vector2
            (
                FormatCoordinate(coords.X, navGrid.MiddleOfMap.X),
                FormatCoordinate(coords.Y, navGrid.MiddleOfMap.Y)
            );
        }
    }
}
