...
namespace CoTG.CoTGServer.GameObjects
{
    public class Buff : IEventSource
    {
        ...
        /// <summary>
        /// StatsModifier that is currently applied to the Target
        /// </summary>
        public StatsModifier? StatsModifier
        {
            get => ScriptInternal?.StatsModifier;
        }

        /// <summary>
        /// Script instance for this buff. Casting to a specific buff class gives access its functions and variables.
        /// </summary>
        public IBuffScript? BuffScript => ScriptInternal;
        internal IBuffScriptInternal? ScriptInternal { get; private set; }
        ...
        public int StackCount => TargetUnit.Buffs.GetStacks(Name, SourceUnit)?.Count ?? 0;
        ...
        )
        {
            ...
            if (script is IBuffScriptInternal buffScriptInternal)
            {
                LoadScript(buffScriptInternal);
            }
        }

        internal void LoadScript(IBuffScriptInternal? script = null)
        {
            // Debug Only
            bool isReload = BuffScript != null;
            if (isReload)
            {
                Deactivate(false);
            }
            ...
            if (script == null)
            {
                var scriptNamespace = "Buffs";
                script = Game.ScriptEngine.CreateObject<IBuffScriptInternal>(scriptNamespace, Name, Game.Config.SupressScriptNotFound);
            }
            if (script == null)
            {
        ...
        internal void Deactivate(bool expired)
        {
            try
            {
                ScriptInternal?.Deactivate(expired);
            }
            catch (Exception e)
            {
                _logger.Error(null, e);
            ...
            if (ScriptInternal?.StatsModifier is not null)
            {
                TargetUnit.RemoveStatModifier(ScriptInternal.StatsModifier);
            }

            ApiEventManager.OnBuffDeactivated.Publish(this);
            ApiEventManager.OnUnitBuffDeactivated.Publish(TargetUnit, this);
            if (ScriptInternal is not null)
            {
                ApiEventManager.RemoveAllListenersForOwner(ScriptInternal);
            }
        }
