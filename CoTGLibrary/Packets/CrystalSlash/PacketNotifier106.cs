...
using ChangeSlotSpellDataType = CoTGEnumNetwork.Enums.ChangeSlotSpellDataType;
using Channel = CoTGEnumNetwork.Packets.Enums.Channel;
using Color = CoTGEnumNetwork.Content.Color;
...
using Talent = CrystalSlash.Game.Common.Talent;
using BasePacket = MirrorImage.BasePacket;
using ChatPacket = CrystalSlash.ChatPacket;
using KeyCheckPacket = CrystalSlash.KeyCheckPacket;
//
...
namespace PacketDefinitions106
{
    ...
    /// </summary>
    public class PacketNotifier106
    {
        ...
        public PacketNotifier106(PacketHandlerManager106 packetHandlerManager, NavigationGrid navGrid)
        {
        ...
        public void NotifyAddRegion(Region region, int userId, TeamId team)
        {
            ...
            if (region.CollisionUnit is not null)
            {
                pkt = new S2C_AddUnitPerceptionBubble()
                {
                    ...
                    PerceptionBubbleType = (uint)region.Type,
 ...
 */
        ...
        private CrystalSlash.Game.Common.CastInfo ConvertCastInfo(CoTG.CoTGServer.GameObjects.SpellNS.CastInfo ci)
        {
            ...
            return new CrystalSlash.Game.Common.CastInfo
            {
            ...
            /* var packet = new S2C_SpawnLevelProp
             {
                 ...
                 Rank = levelProp.Rank,
                 Type = (byte)((int)levelProp.Type - 1),
                 Name = levelProp.Name,
            ...
            /*   var packet = new S2C_CHAR_SpawnPet
               {
                   ...
                   SenderNetID = pet.SourceSpell.Caster.NetId,
                   ClearFocusTarget = true, //TODO
               ...
               SendSpawnPacket((int)pet.SourceSpell.Caster.NetId, pet, packet, true);
            */
            //hack actually this packet is totally fucked up on 106 
        ...
        private BasicAttackData CreateBasicAttackData(CoTG.CoTGServer.GameObjects.SpellNS.CastInfo castInfo)
        {
        ...
        /// </summary>
        public void NotifyBasic_Attack(CoTG.CoTGServer.GameObjects.SpellNS.CastInfo castInfo)
        {
        ...
        /// </summary>
        public void NotifyBasic_Attack_Pos(CoTG.CoTGServer.GameObjects.SpellNS.CastInfo castInfo)
        {
            var attacker = castInfo.Caster;
        ...
        public void NotifyChangeSlotSpellData(int userId, ObjAIBase owner, int slot, ChangeSlotSpellDataType changeType, bool isSummonerSpell = false, TargetingType targetingType = TargetingType.Invalid, string newName = "", float newRange = 0, float newMaxCastRange = 0, float newDisplayRange = 0, int newIconIndex = 0x0, List<uint> offsetTargets = null)
        {
            ...
            /* case ChangeSlotSpellDataType.Range:
                 {
                     var spellData = new ChangeSpellDataRange()
                     {
                         SpellSlot = slot,
                         IsSummonerSpell = isSummonerSpell,
                         CastRange = newRange
             ...
             case ChangeSlotSpellDataType.MaxGrowthRange:
                 {
                     var spellData = new ChangeSpellDataMaxGrowthRange()
                     {
                         SpellSlot = slot,
                         IsSummonerSpell = isSummonerSpell,
             ...
             case ChangeSlotSpellDataType.RangeDisplay:
                 {
                     var spellData = new ChangeSpellDataRangeDisplay()
                     {
                         SpellSlot = slot,
                         IsSummonerSpell = isSummonerSpell,
        ...
        */
        ...
        S2C_OnEnterVisiblityClient ConstructEnterVisibilityClientPacket(GameObject o, List<BasePacket>? packets = null)
        {
            ...
            {
                md = PacketExtensions106.CreateMovementDataWithSpeedIfPossible(u, _navGrid, useTeleportID: true);
            }
            else if (o is Pet p)
            {
                md = PacketExtensions106.CreateMovementDataWithSpeedIfPossible(p, _navGrid, useTeleportID: false);
            }
            else
            {
                md = PacketExtensions106.CreateMovementDataStop(o);
            }
        ...
        public void NotifyEnterVisibilityClient(GameObject o, int userId = -1, bool ignoreVision = false, List<BasePacket> packets = null)
        {
            ...
            else
            {
                ...
                {
                    _packetHandlerManager.BroadcastPacketVision(o, enterVis, Channel.CHL_S2C);
                }
            }
        ...
        public void NotifyLoadScreenInfo(int userId, TeamId team, List<ClientInfo> players)
        {
            ...
            var teamRoster = new CrystalSlash.LoadScreen.TeamRosterUpdate
            {
                TeamSizeOrder = 6,
                TeamSizeChaos = 6
            };
        ...
        public void NotifyNPC_CastSpellAns(CoTG.CoTGServer.GameObjects.SpellNS.CastInfo ci)
        {
            //throw new InvalidOperationException("for trace");
        ...
        public void NotifyNPC_Die_Broadcast(DeathData data)
        {
            var dieMapView = new S2C_NPC_Die
            {
                ...
                DeathData = new CrystalSlash.Game.Common.DeathData
                {
        ...
        public void NotifyNPC_Hero_Die(DeathData deathData)
        {
            ...
            //NotifyOnEventDie(deathData, champ);
            ...
            uint missileNetID = 0)
        {
            ...
            //  _packetHandlerManager.BroadcastPacketVision(attacker, stopAttack, Channel.CHL_S2C);
        }
            ...
            if (u.Replication != null)
            {
                var us = new S2C_OnReplication()
                {
                    ...
                    ReplicationData = new List<ReplicationData>(1){
                        u.Replication.GetData106(false) //partial
                    }
                };
        ...
        public void NotifyPingLoadInfo(ClientInfo client, C2S_Ping_Load_Info packet)
        {
            ...
            //var response = new CrystalSlash.Game.S2C_Ping_Load_Info
            //{
            //    ConnectionInfo = new CrystalSlash.Game.Common.ConnectionInfo
            //    {
        ...
        {
            var loadName = new CrystalSlash.LoadScreen.RequestRename
            {
        ...
        {
            var loadChampion = new CrystalSlash.LoadScreen.RequestReskin
            {
                PlayerID = (ulong)player.PlayerId,
                SkinID = (uint)player.SkinNo,
        ...
        public void NotifyS2C_AmmoUpdate(Spell spell)
        {
            ...
            //doesn't exist in .106 ammo method 
        ...
        {
            var response = new CrystalSlash.Game.S2C_QueryStatusAns
            {
                IsOK = true
            };
            ...
            /* if(slot < 4)//just an test for see if it crash
             {
                 var spellDataPacket = new S2C_SetSpellData
                 {
                     SenderNetID = netId,
                     UnitNetID = netId,
                     SpellNameHash = (uint)HashString(spellName),
                     SpellSlot = (byte)slot
                 };
        ...
        {
            var start = new CrystalSlash.Game.S2C_StartSpawn
            {
                BotCountOrder = 0,
                BotCountChaos = 0
            };
        ...
        public void NotifyS2C_UnitSetLookAt(AttackableUnit attacker, LookAtType lookAtType, AttackableUnit? target, Vector3 targetPosition = default)
        {
            ...
            var packet = new S2C_FaceDirection
            {
                ...
                Direction = target?.Position3D ?? targetPosition,
        ...
        {
            var endSpawnPacket = new CrystalSlash.Game.S2C_EndSpawn();
        ...
        public void NotifyEnterTeamVision(AttackableUnit u, int userId = -1, BasePacket? spawnPacket = null)
        {
            if (u is Pet p)
            {
                ...
                Console.WriteLine("PET IS INVOKED by " + p.SourceSpell.Caster.Name);
                if (spawnPacket is not S2C_OnEnterVisiblityClient visibilityPacket)
                {
                    List<BasePacket>? packets = null;

                    visibilityPacket = ConstructEnterVisibilityClientPacket(p, packets);
                }

                S2C_OnEnterLocalVisiblityClient healthbarPacket = new()
                {
                    SenderNetID = p.NetId,
                    MaxHealth = p.Stats.HealthPoints.Total,
                ...
                /*  var vision = new S2C_OnEnterTeamVisiblity
                  {
                      VisiblityTeam = 1,
                      SenderNetID = p.NetId,
                ...
                */
                ...
                //  _packetHandlerManager.SendPacket(userId, vision, Channel.CHL_S2C);
            }
            else
            {
                if (spawnPacket is not S2C_OnEnterVisiblityClient visibilityPacket)
            ...
            if (u.Replication is not null)
            {
                ...
                S2C_OnReplication us = new()
                {
                    ...
                    ReplicationData = new List<ReplicationData>(1)
                    {
                        u.Replication.GetData106(false)
                    }
                };
                if (u.Replication.Values != null)
                {
                    if (u is Pet p2)
                    {
                        ...
                        _packetHandlerManager.SendPacket(-1, us, Channel.CHL_S2C);
                    }
                    else
                    {
        ...
        {
            var data = PacketExtensions106.CreateMovementDataNormal(u, _navGrid, useTeleportID);
        ...
        {
            var data = u.Replication.GetData106(partial);
        ...
        public void NotifyWaypointGroupWithSpeed(AttackableUnit u)
        {
            ...
            var md = PacketExtensions106.CreateMovementDataWithSpeed(u, _navGrid);
            ...
            _packetHandlerManager.BroadcastPacketVision(u, speedWpGroup, Channel.CHL_S2C);
        }
     ...
     (
     ...
     )
        {
            ...
            _packetHandlerManager.BroadcastPacketVision(u, speedWpGroup, Channel.CHL_LOW_PRIORITY);
        }
        public void NotifyWaypointList(GameObject obj, List<Vector2> waypoints)
        {
            ...
            _packetHandlerManager.BroadcastPacketVision(obj, wpList, Channel.CHL_S2C);
        }
        ...
        public void NotifyWorld_SendCamera_Server_Acknowledgement(ClientInfo client, CrystalSlash.Game.C2S_World_SendCamera_Server request)
        {
        ...
        public void NotifyAttachFlexParticle(ObjAIBase unit, byte _ParticleFlexID, byte _CpIndex, uint _ParticleAttachType)
        {
            /* var AttachFlexParticle = new S2C_AttachFlexParticle
             ...
             _packetHandlerManager.BroadcastPacket(AttachFlexParticle, Channel.CHL_S2C, PacketFlags.NONE);*/
        }
        /// <summary>
        /// todo:
        /// </summary>
