...
using CoTGEnumNetwork.Packets.PacketDefinitions;
...
namespace PacketDefinitions106
{
    ...
    /// </summary>
    public class PacketHandlerManager106
    {
        // Logger
        ...
        public PacketHandlerManager106(BlowFish[] blowfishes, Host server)
        {
        ...
        private static void Log(string str, object packet)
        {
            ...
            if (Game.Config.EnableLogPKT)
            {
                if (packet.GetType().Name == "S2C_WaypointGroup")
                {
                    ...
                    {
                        Console.WriteLine(" x = " + ((new Vector2(wp.X, wp.Y) * 2) + new Vector2(6991.434f, 7223.343f)).X + " Y :" + ((new Vector2(wp.X, wp.Y) * 2) + new Vector2(6991.434f, 7223.343f)).Y);
        ...
        {
            foreach (var m in typeof(PacketReader106).GetMethods())
            {
                foreach (Attribute attr in m.GetCustomAttributes(true))
                {
                    if (attr is PacketType106)
                    {
                        if (((PacketType106)attr).ChannelId == Channel.CHL_LOADING_SCREEN)
                        {

                            var method = (RequestConvertor)Delegate.CreateDelegate(typeof(RequestConvertor), m);

                            LoadScreenConvertorTable.Add(((PacketType106)attr).LoadScreenPacketId, method);
                        }
                        if (((PacketType106)attr).ChannelId == Channel.CHL_COMMUNICATION)
                        {
                            var method = (RequestConvertor)Delegate.CreateDelegate(typeof(RequestConvertor), m);
                        ...
                        {
                            var key = new Tuple<GamePacketID, Channel>(((PacketType106)attr).GamePacketId, ((PacketType106)attr).ChannelId);
                            var method = (RequestConvertor)Delegate.CreateDelegate(typeof(RequestConvertor), m);
        ...
        public bool SendPacket(int userId, byte[] source, Channel channelNo, PacketFlags flag = PacketFlags.RELIABLE, BasePacket packetid = default)
        {
            ...
            if (Game.Config.EnableReplay)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        float timestamp2 = Game.Time.DeltaTimeSeconds;
        ...
        public static ulong Encrypt106(State state, ulong block)
        {
        ...
        public static ulong Decrypt106(State state, ulong block)
        {
