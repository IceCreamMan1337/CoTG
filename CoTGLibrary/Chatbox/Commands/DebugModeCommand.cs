...
{
    public class DebugModeCommand : ChatCommandBase
    {
        private static ILog _logger = LoggerProvider.GetLogger();
        private float lastDrawTime;
        private int _userId;
        private Champion _userChampion;
        private static readonly Dictionary<uint, Particle> _circleParticles = new();
        private static readonly Dictionary<uint, List<Particle>> _arrowParticlesList = new();
        [Flags]
        private enum DebugMode : int
        {
            None = 1 << 0, Self = 1 << 1, Champions = 1 << 2, Minions = 1 << 3, Projectiles = 1 << 4, /*Sectors,*/ PathFinding = 1 << 6, All = 1 << 7
        }
        ...
        public DebugModeCommand(ChatCommandManager chatCommandManager) : base(chatCommandManager)
        {
        ...
        private Particle AddParticle(GameObject bindObj, Vector2 startPos, string particleName, float scale, Vector3 direction)
        {
            return ApiFunctionManager.AddParticleTarget(bindObj, particleName, bindObj, startPos, 0.200f, scale, direction);
        }

        public override void Execute(int userId, bool hasReceivedArguments, string arguments = "")
        {
            _userId = userId;
            _userChampion = Game.PlayerManager.GetPeerInfo(userId).Champion;

            var split = arguments.ToLower().Split(' ');

            if (split.Length <= 1)
            {
                ChatManager.System($"Currently debugging {_debugMode & ~DebugMode.None}.");
                return;
            }
            ...
            if (Enum.TryParse(typeof(DebugMode), split[1], true, out var requestDbgMode))
            {
                ...
                if (requestedDebugMode != DebugMode.None)
                {
                    ...
                    else
                    {
                        ...
                        ChatManager.System(startdebugmsg);
                        CreateUnitHighlightNotify(userId, _userChampion);
                ...
                if (requestedDebugMode == 0 || _debugMode == DebugMode.None)
                {
                    ...
                    ChatManager.System(message);
                    _logger.Debug(message);
                    RemoveUnitHighlightNotify(userId, _userChampion);
        ...
        void DrawAttackableUnit(AttackableUnit u, int userId = -1)
        {
            ...
            //Game.PacketNotifier126.NotifyFXCreateGroup(circleparticle, userId);
            ...
            if (u.Waypoints.Count > 0)
            {
                ...
                for (int waypoint = u.CurrentWaypointKey; waypoint < u.Waypoints.Count; waypoint++)
                {
                    ...
                    //Game.PacketNotifier126.NotifyFXCreateGroup(arrowparticle, userId);
        ...
        public void DrawMinions(int userId)
        {
            ...
            foreach (GameObject obj in Game.ObjectManager.GetObjects().Values)
            {
                ...
                if (obj.Value is SpellMissile missile)
                {
                    ...
                    if (missile.CastInfo.Targets.Any() || (missile.CastInfo.TargetPosition != Vector3.Zero && missile.CastInfo.TargetPositionEnd != Vector3.Zero))
                    {
                        ...
                        else if (missile is SpellCircleMissile skillshot)
                        {
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleStart, userId);
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleEnd, userId);
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleEnd2Temp, userId);
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleStart2, userId);
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleEnd3Temp, userId);
                            ...
                            //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleStart3, userId);
                        }
                    }
                }
            }
                    ...
                    if (sector.CurrentCastInfo.Target != null || (sector.CastInfo.TargetPosition != Vector3.Zero && sector.CastInfo.TargetPositionEnd != Vector3.Zero))
                    {
                        ...
                        if (sector is SpellSectorPolygon polygon)
                        {
                            ...
                            foreach (Vector2 vert in polygon.GetPolygonVertices())
                            {
                                ...
                                //Game.PacketNotifier126.NotifyFXCreateGroup(arrowParticleVert, userId);
                            }
                        }
                    }
                }
        ...
        */
        ...
        {
            DrawSelf(userId);
            foreach (var cell in Game.Map.NavigationGrid.GetAllCellsInRange(_userChampion.Position, 500))
            {
                var cp = AddParticle(_userChampion, Game.Map.NavigationGrid.TranslateFromNavGrid(cell.GetCenter()),
