...
multikill
    static Dictionary<NeutralMinionCamp, Monster> SpeedShrines = new();
    static Dictionary<NeutralMinionCamp, Monster> HealthPacks = new();
    static List<Crystal> Crystals = new();

    public static void InitializeNeutrals()
    {
        SetupCamps();

        Crystals.Add(new Crystal(new Vector2(7074.9736f, 6462.0273f), TeamId .TEAM_ORDER));
        Crystals.Add(new Crystal(new Vector2(6801.1855f, 6462.0273f), TeamId .TEAM_CHAOS));

        foreach (var camp in SpeedShrines.Keys)
        {
            if (!camp.IsAlive)
            {
                AddPosPerceptionBubble(camp.Position.ToVector2(), 250.0f, 1.0f, TeamId .TEAM_ORDER);
                AddPosPerceptionBubble(camp.Position.ToVector2(), 250.0f, 1.0f, TeamId .TEAM_CHAOS);
...
public class Crystal
{
    ...
    public void SpawnCrystal()
    {
        ...
        Regions.Add(AddUnitPerceptionBubble(crystal, 350.0f, 25000.0f, TeamId .TEAM_ORDER, collisionArea: 38.08f));
        Regions.Add(AddUnitPerceptionBubble(crystal, 350.0f, 25000.0f, TeamId .TEAM_CHAOS, collisionArea: 38.08f));
