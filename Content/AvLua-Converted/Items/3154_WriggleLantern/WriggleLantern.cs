...
namespace Buffs
{
    public class wrigglelantern : BuffScript
    {
        ...
        {
            if (target is ObjAIBase && RandomChance() < 0.2f && target is not LaneTurret && target is not Champion && GetBuffCountFromCaster(target, default, nameof(Buffs.OdinGolemBombBuff)) == 0)
            {
