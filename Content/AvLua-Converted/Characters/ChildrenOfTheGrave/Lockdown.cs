namespace Spells
{
    public class Lockdown : SpellScript
    {
        ...
        public override void ChannelingStart()
        {
            ...
            SpellEffectCreate(out particleID, out _, "Lockdown_mis.troy", default, TeamId.TEAM_UNKNOWN, 0, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, "head", default, target, "root", default, false, false, false, false, false);
            AddBuff(owner, target, new Buffs.Lockdown(nextBuffVars_DamageToDeal), 1, 1, 2.5f, BuffAddType.REPLACE_EXISTING, BuffType.DAMAGE, 0, true, false, false);
...
namespace Buffs
{
    public class Lockdown : BuffScript
    {
        public override BuffScriptMetadataUnmutable MetaData { get; } = new()
        {
            AutoBuffActivateAttachBoneName = new[] { "", "root", },
            AutoBuffActivateEffect = new[] { "", "Lockdown_tar.troy", },
            BuffName = "Lockdown",
            BuffTextureName = "Lockdown.dds",
        };
        float damageToDeal;
        float ticksRemaining;
        float lastTimeExecuted;
        public Lockdown(float damageToDeal = default)
        {
        ...
        public override void OnUpdateActions()
        {
            ...
            {
                if (ExecutePeriodically(1.0f, ref lastTimeExecuted, false))
                {
                    ApplyDamage(attacker, target, damageToDeal, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELLPERSIST, 1, 0.26f, 0, false, false, attacker);
                    ticksRemaining--;
                }
