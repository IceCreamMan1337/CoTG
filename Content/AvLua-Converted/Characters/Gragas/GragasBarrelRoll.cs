...
namespace Buffs
{
    public class GragasBarrelRoll : BuffScript
    {
        ...
        {
          //  Console.WriteLine("GragasBarrelRollMissile has end");
            if (spellName == nameof(Spells.GragasBarrelRollMissile))
            {
              //  Console.WriteLine("GragasBarrelRollMissile has end 2");
                TeamId teamID = GetTeamID_CS(owner);
                int gragasSkinID = GetSkinID(owner);
                Vector3 targetPos = this.targetPos;
                Minion other1 = SpawnMinion("DoABarrelRoll", "TestCube", "idle.lua", targetPos, teamID, false, true, false, false, true, true, 0, default, true, (Champion)owner);
