...
namespace Buffs
{
    public class MoltenShield : BuffScript
    {
        ...
        public override void OnPreDamage2(AttackableUnit target, ref float damageAmount, DamageType damageType, DamageSource damageSource)
        {
            if (owner.Team != target.Team && ( damageSource == DamageSource.DAMAGE_SOURCE_ATTACK || damageSource == DamageSource.DAMAGE_SOURCE_DEFAULT))//&& damageSource == default)
            {
                ...
                ApplyDamage((ObjAIBase)owner, target, damageReturn, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELLAOE, 1, 0.2f, 1, false, false, (ObjAIBase)owner);
            }
        }
    }
}