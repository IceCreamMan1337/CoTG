namespace Spells
{
    public class DefensiveBallCurl : SpellScript
    {
        ...
        private readonly int[] armorBonusByLevel = { 50, 75, 100, 125, 150 };
        private readonly int[] damageReturnByLevel = { 15, 25, 35, 45, 55 };

        public override void SelfExecute()
        {
            float armorBonus = armorBonusByLevel[level - 1];
            float damageReturn = damageReturnByLevel[level - 1];
            AddBuff(attacker, owner, new Buffs.DefensiveBallCurl(armorBonus, damageReturn), 1, 1, 6, BuffAddType.REPLACE_EXISTING, BuffType.COMBAT_ENCHANCER, 0, true, false, false);
        }
    }
}
...
namespace Buffs
{
    public class DefensiveBallCurl : BuffScript
    {
        ...
        {
            AutoBuffActivateEffect = new[] { "DefensiveBallCurl_buf", },
            BuffName = "DefensiveBallCurl",
            BuffTextureName = "Armordillo_ShellBash.dds",
        };

        private readonly float armorAmount;
        private readonly float damageReturn;
        ...
        public override void OnDeactivate(bool expired)
        {
            ...
            float baseCooldown = 14;
            float cooldownModifier = 1 + GetPercentCooldownMod(owner);
            float newCooldown = baseCooldown * cooldownModifier - lifeTime;
        ...
        {
            if (attacker is not LaneTurret)
            {
                float baseArmor = GetArmor(owner);
                float damageReturned = baseArmor * 0.1f + damageReturn;
                ApplyDamage((ObjAIBase)owner, attacker, damageReturned, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELLAOE, 1, 0, 1, false, false, (ObjAIBase)owner);
