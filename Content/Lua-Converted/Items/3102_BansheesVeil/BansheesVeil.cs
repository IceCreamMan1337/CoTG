...
namespace Buffs
{
    public class BansheesVeil : BuffScript
    {
        ...
        {
            string effectName = GetBuffCountFromCaster(owner, owner, nameof(Buffs.XerathAscended)) > 0
                ? "bansheesveil_buf_tempXerath.troy"
                : "bansheesveil_buf.troy";

            SpellEffectCreate(out shieldEffect, out _, effectName, default, TeamId.TEAM_UNKNOWN, 0, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, "C_BUFFBONE_GLB_CENTER_LOC", default, owner, default, default, false, true, false, false, false);
        }

        public override void OnDeactivate(bool expired)
        {
            SpellEffectRemove(shieldEffect);
        }
        ...
        {
            if (owner.Team != attacker.Team && !GetIsAttackOverride(spell) && !spellVars.DoesntBreakShields && !spellVars.DoesntTriggerSpellCasts)
            {
                willRemove = true;
                TriggerShieldEffect();
            }
