    ...
    public class CharScriptKarma : CharScript
    {
        ...
        public override void OnSpellCast(Spell spell, string spellName, SpellScriptMetadata spellVars)
        {
            ...
            if (spellSlot != 3 && GetBuffCountFromCaster(owner, owner, nameof(Buffs.KarmaChakra)) > 0)
            {
                ...
                float multiplier;
                float newCooldown;
                //error in class ReusableAwaiter<T> : INotifyCompletion // actually buff seem removed when the spell is casted , so we have an awaiter error 
        ...
        public override void OnActivate()
        {
            ...
            // at first view onactivate need be activated when champion has already spawn not the invert 
