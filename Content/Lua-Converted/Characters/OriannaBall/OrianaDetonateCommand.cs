...

        {
            ...
            if (!deployed)
            {
                if (GetBuffCountFromCaster(owner, default, nameof(Buffs.OriannaBallTracker)) > 0)
                {
                    ...
                    //hack 
                    charVars.UltimateType = currentType;
                }
                else
                {
            ...
            foreach (AttackableUnit unit in GetUnitsInArea(owner, targetPos, rangeVar, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectMinions | SpellDataFlags.AffectHeroes, default, true))
            {
                ...
                if (validTarget)
                {
                    ...
                    // Calculate direction from unit to ball position
                    Vector3 unitPos = GetUnitPosition(unit);
                    Vector3 directionToBall = Vector3.Normalize(targetPos - unitPos);

                    // Move unit a fixed distance towards the ball
                    float fixedDistance = 400f;
                    Vector3 displacementTarget = unitPos + (directionToBall * fixedDistance);

                    // Apply stun and displacement
                    AddBuff(owner, unit, new Buffs.OrianaStun(), 1, 1, 0.75f, BuffAddType.REPLACE_EXISTING, BuffType.STUN, 0, true, false, false);
                    AddBuff(owner, unit, new Buffs.Move(775f, 25f, displacementTarget), 1, 1, 0.75f, BuffAddType.REPLACE_EXISTING, BuffType.STUN, 0, true, false, false);
                }
            }
        }
    }
