    ...
    public class JarvanIVCataclysm : SpellScript
    {
            ...
            ref HitResult hitResult)
        {
            ...
            else
            {
                ...
                // Move(owner, targetPos, 2000, 150, 0, ForceMovementType.FURTHEST_WITHIN_RANGE, ForceMovementOrdersType.CANCEL_ORDER, 0, ForceMovementOrdersFacing.FACE_MOVEMENT_DIRECTION);
    ...
    public class JarvanIVCataclysm : BuffScript
    {
        ...
        public override void OnUpdateActions()
        {
            if (!hasDealtDamage)
            {
                ...
                if (distance <= 500 && owner.MovementParameters == null)
                {
                    SetCanCast(owner, true);
                    hasDealtDamage = true;
        ...
        public override void OnMoveSuccess()
        {
            if (!hasCreatedRing)
            {
                ...
                foreach (Vector3 pos in GetPointsAroundCircle(centerPos, 350, 12))
                {
                    ...
                    Minion other2 = SpawnMinion("JarvanIVWall", "JarvanIVWall", "idle.lua", pos, teamID, true, true, true, true, true, false, 0, false, false, (Champion)owner);
                    // other2.IsWallMinion = true;
