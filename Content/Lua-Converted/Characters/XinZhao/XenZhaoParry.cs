namespace Spells
{
    public class XenZhaoParry : SpellScript
    {
        ...
        {
            Vector3 castPos = owner.Position3D; // UNITIALIZED
            float weaponDmgBonus = this.effect2[level - 1]; ; // UNITIALIZED  // this.effect2[level - 1]; ? 
            float dtD = this.effect0[level - 1];
            float percentByLevel = this.effect1[level - 1];
            SpellEffectCreate(out _, out _, "xenZiou_ult_cas.troy", default, TeamId.TEAM_NEUTRAL, 900, 0, TeamId.TEAM_UNKNOWN, default, owner, false, owner, "BUFFBONE_CSTM_WEAPON_1", castPos, owner, default, default, true, false, false, false, false);
            ...
            AddBuff(owner, owner, new Buffs.XenZhaoParry(nextBuffVars_MRByLevel, nextBuffVars_TotalMR), 1, 1, 6, BuffAddType.REPLACE_EXISTING, BuffType.COMBAT_ENCHANCER, 0, true, false, false);
            ...
            AddBuff(owner, owner, new Buffs.XenZhaoSweepArmor(nextBuffVars_TotalArmor), 1, 1, 6, BuffAddType.REPLACE_EXISTING, BuffType.INTERNAL, 0, true, false, false);
        }
