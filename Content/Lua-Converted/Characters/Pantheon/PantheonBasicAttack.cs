namespace Spells
{
    public class PantheonBasicAttack : SpellScript
    {
        ...
        {
            if (hitResult != HitResult.HIT_Dodge && hitResult != HitResult.HIT_Miss && GetBuffCountFromCaster(owner, owner, nameof(Buffs.Pantheon_CertainDeath)) > 0 && target is ObjAIBase && target is not LaneTurret)
            {
            ...
            //was getbaseattackdamage , seem not correct
...
namespace PreLoads
{
    public class PantheonBasicAttack : IPreLoadScript
    {
        public void Preload()
        {
