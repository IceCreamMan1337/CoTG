    ...
    public class AhriFoxFireMissile : SpellScript
    {
        ...
        public override void OnMissileUpdate(SpellMissile missileNetworkID, Vector3 missilePosition)
        {
            ...
            if (spellVars_Ready >= 3)
            {
                ...
                {
                    SpellCast(owner, unit, default, default, 3, SpellSlotType.ExtraSlots, level, true, true, false, true, false, true, missilePosition);
                    count = 1;
                ...
                if (count == 0)
                {
                    foreach (AttackableUnit unit in GetClosestVisibleUnitsInArea(owner, missilePosition, 650, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectMinions | SpellDataFlags.AffectHeroes, 1, default, true))
                    {
                        ...
                        {
                            AddBuff(owner, unit, new Buffs.AhriFoxFireMissileTag(), 1, 1, 0.25f, BuffAddType.REPLACE_EXISTING, BuffType.INTERNAL, 0, true, false, false);
                        }
                        else
                        {
                            AddBuff(owner, unit, new Buffs.AhriFoxFireMissileTagTwo(), 1, 1, 0.25f, BuffAddType.REPLACE_EXISTING, BuffType.INTERNAL, 0, true, false, false);
                        }
                    }
                }
            }
