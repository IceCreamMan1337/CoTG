namespace Spells
{
    public class Destiny : SpellScript
    {
        ...
        private readonly int[] buffDurationByLevel = { 6, 8, 10 };

        public override void SelfExecute()
        {
            int duration = buffDurationByLevel[level - 1];
            AddBuff(owner, owner, new Buffs.Destiny_marker(), 1, 1, duration, BuffAddType.REPLACE_EXISTING, BuffType.COMBAT_ENCHANCER, 0, true, false, false);
            ...
            foreach (AttackableUnit unit in GetUnitsInArea(owner, owner.Position3D, 25000, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectHeroes, default, true))
            {
                ...
                AddBuff(attacker, unit, new Buffs.Destiny(), 1, 1, duration, BuffAddType.REPLACE_EXISTING, BuffType.COMBAT_DEHANCER, 0, true, false, false);
            }

            // Set the spell to "Gate" and set cooldown time
