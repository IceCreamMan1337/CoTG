    ...
    public class PickACard : SpellScript
    {
            ...
            ref HitResult hitResult)
        {
            ...
            if (rnd1 < 0.34f)
            {
                ...
                SetSpell(owner, 1, SpellSlotType.SpellSlots, SpellbookType.SPELLBOOK_CHAMPION, nameof(Spells.BlueCardLock));
                SetSlotSpellCooldownTime(owner, 1, SpellbookType.SPELLBOOK_CHAMPION, SpellSlotType.SpellSlots, 0.005f);
            }
            else if (rnd1 < 0.67f)
            {
                nextBuffVars_Counter = 2;
                SetSpell(owner, 1, SpellSlotType.SpellSlots, SpellbookType.SPELLBOOK_CHAMPION, nameof(Spells.RedCardLock));
                SetSlotSpellCooldownTime(owner, 1, SpellbookType.SPELLBOOK_CHAMPION, SpellSlotType.SpellSlots, 0.005f);
            }
            else
            {
                nextBuffVars_Counter = 4;
                SetSpell(owner, 1, SpellSlotType.SpellSlots, SpellbookType.SPELLBOOK_CHAMPION, nameof(Spells.GoldCardLock));
                SetSlotSpellCooldownTime(owner, 1, SpellbookType.SPELLBOOK_CHAMPION, SpellSlotType.SpellSlots, 0.005f);
            }
            bool nextBuffVars_WillRemove = false; // UNUSED
            AddBuff(owner, target, new Buffs.PickACard_tracker(), 1, 1, 10, BuffAddType.REPLACE_EXISTING, BuffType.COMBAT_ENCHANCER, 0, true, false, false);
            AddBuff(owner, target, new Buffs.PickACard(nextBuffVars_Counter), 1, 1, 10, BuffAddType.REPLACE_EXISTING, BuffType.INTERNAL, 0, true, false, false);
        }
    }
}
namespace Buffs
               ...
               public override void OnPreAttack(AttackableUnit target)
              {
                  if (target is ObjAIBase && frozen == 1)
                  {
                     // SkipNextAutoAttack((ObjAIBase)owner);
                      ...
                      if (counter <= 1)
                      {
                          SpellCast((ObjAIBase)owner, target, target.Position3D, target.Position3D, 7, SpellSlotType.ExtraSlots, level, true, false, false, true, true, false);
                      }
                      else if (counter <= 3)
                      {
                          SpellCast((ObjAIBase)owner, target, target.Position3D, target.Position3D, 8, SpellSlotType.ExtraSlots, level, true, false, false, true, true, false);
                      }
                      else
                      {
