...
namespace Buffs
{
    public class ShenShadowDash : BuffScript
    {
        ...
        {
            if (owner.Team != target.Team && target is ObjAIBase && target is not LaneTurret && GetBuffCountFromCaster(target, default, nameof(Buffs.SharedWardBuff)) == 0 && GetBuffCountFromCaster(target, owner, nameof(Buffs.ShenShadowDashCooldown)) == 0)
            {
