namespace Buffs
{
    public class LeonaSunlightPassive : BuffScript
    {
        ...
        {
            if (hitResult != HitResult.HIT_Dodge && hitResult != HitResult.HIT_Miss && GetBuffCountFromCaster(owner, owner, nameof(Buffs.LeonaSolarBarrierTracker)) > 0 && target is ObjAIBase && target is not LaneTurret)
            {
