...
namespace Buffs
{
    public class SiphoningStrikeNew : BuffScript
    {
            ...
            DamageSource damageSource, ref HitResult hitResult)
        {
            ...
            //hack , missthetiming
            SpellBuffRemove(owner, nameof(Buffs.SiphoningStrikeNew), (ObjAIBase)owner);
            //SpellBuffClear(owner, nameof(Buffs.SiphoningStrikeNew));
            //newest suposition , spellbuffclear mean = doesn't send pkt ??? 

            //was DamageSource.DAMAGE_SOURCE_ATTACK this is logical 
            //so for moment we put periodic damage , and we will see what happen ( sheen seem work properly ) 

            ApplyDamage(attacker, target, damageToDeal, DamageType.DAMAGE_TYPE_PHYSICAL, DamageSource.DAMAGE_SOURCE_PERIODIC, 1, 0, 0, false, true, attacker);
            damageAmount *= 0;
