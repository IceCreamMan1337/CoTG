namespace Spells
{
    public class LuxMaliceCannonPartFix : SpellScript
    {
...
namespace Buffs
{
    public class LuxMaliceCannonPartFix : BuffScript
    {
        public override void OnActivate()
        {
            Vector3 ownerPos = GetUnitPosition(owner); // UNUSED
            TeamId teamID = GetTeamID_CS(owner);
            Vector3 beam1 = GetPointByUnitFacingOffset(owner, 471, 0);
            Vector3 beam2 = GetPointByUnitFacingOffset(owner, 942, 0);
            Vector3 beam3 = GetPointByUnitFacingOffset(owner, 1413, 0);
