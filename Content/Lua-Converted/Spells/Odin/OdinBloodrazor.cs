namespace Buffs
{
    public class OdinBloodrazor : BuffScript
    {
        public override void OnHitUnit(AttackableUnit target, ref float damageAmount, DamageType damageType,
            DamageSource damageSource, ref HitResult hitResult)
        {
            if (target is ObjAIBase && target is not LaneTurret && hitResult != HitResult.HIT_Dodge && hitResult != HitResult.HIT_Miss)
            {
                float maxHealth = GetMaxHealth(target, PrimaryAbilityResourceType.MANA);
                TeamId teamId = GetTeamID_CS(target);
                float damage = 0.025f * maxHealth;
