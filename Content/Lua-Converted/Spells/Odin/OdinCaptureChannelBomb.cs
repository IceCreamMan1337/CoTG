namespace Spells
{
    public class OdinCaptureChannelBomb : SpellScript
    {
        ...
        public override void ChannelingUpdateStats()
        {
            ...
            {
                float accumTime = owner.ChannelSpell.CurrentChannelDuration;
                // UNITIALIZED
                if (accumTime > 1.5f)
                {
            ...
            if (this.chargeTimePassed == 1 && GetBuffCountFromCaster(owner, owner, nameof(Buffs.OdinCaptureChannelBomb)) > 0)
            {
                ...
                if (count == 0)
                {
                    TeamId teamOfOwner = owner.Team;
                    if (teamOfOwner == TeamId.TEAM_ORDER)
                    {
                        AddBuff(owner, target, new Buffs.OdinBombSuppressionOrder(), 10, 1, 30, BuffAddType.STACKS_AND_RENEWS, BuffType.INTERNAL, 0, true, false, false);
                    }
                    else
                    {
                        AddBuff(owner, target, new Buffs.OdinBombSuppressionChaos(), 10, 1, 30, BuffAddType.STACKS_AND_RENEWS, BuffType.INTERNAL, 0, true, false, false);
                    }
                    AddBuff(owner, target, new Buffs.OdinBombSuppression(), 1, 1, 10, BuffAddType.STACKS_AND_OVERLAPS, BuffType.AURA, 0, true, false, false);
                }
            }
        }
