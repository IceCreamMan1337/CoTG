...
class Garen: BehaviourTree
{
    ...
    public Garen() // Ajoutez un constructeur sans param�tres
    {
        // Initialisez les propri�t�s sp�cifiques � Annie ici, si n�cessaire.
    }

    public Garen(Champion owner) : base(owner)
    {
            ...
            ForEach(TargetCollection, Unit => (
                ...
                (
                    (
                        ...
                        MultiplyFloat(out TotalUnitStrength, TotalUnitStrength, 20)
                    ) ||
                    (
                        GetUnitType(out UnitType, Unit) &&
                        UnitType == HERO_UNIT &&
                        MultiplyFloat(out TotalUnitStrength, TotalUnitStrength, 30)
                    ) ||
                    (
                        GetUnitType(out UnitType, Unit) &&
                        UnitType == TURRET_UNIT &&
                        MultiplyFloat(out TotalUnitStrength, TotalUnitStrength, 90)
                    )
                )
            ))
        );
                ...
                (
                    ((
                        ...
                        (
                            ...
                            MultiplyFloat(out StrengthRatioOverTime, StrengthRatioOverTime, StrRatio) &&
                            GetUnitAIAttackers(out TargetCollection, Self) &&
                            (TargetCollection.Any(Unit => (
                                GetUnitType(out UnitType, Unit) &&
                                UnitType == TURRET_UNIT &&
                                MultiplyFloat(out StrengthRatioOverTime, StrengthRatioOverTime, 8)
                            )) || true)
                        ) &&
                        GetGameTime(out PrevTime)
                    ) || true) &&
                    ((
                        ...
                        NewDamage > 0 &&
                        MultiplyFloat(out AccumulatedDamage, AccumulatedDamage, NewDamage)
                    ) || true) &&
                    GetUnitCurrentHealth(out PrevHealth, Self)
                )
            )
    ...
    bool GarenPushLane()
    {
        return
        (
        DebugAction("im GAREN et je push batard") &&
            ClearUnitAIAttackTarget() &&
            IssueMoveOrder() &&
