 ...
 TechmaturgicalRepairBot.Game.Common
{
    ...
    static class PlayerLoadInfoExtension
    {
...

        {
            ...
            {
                data.BotName = reader.ReadFixedString(128);
                data.BotSkinName = reader.ReadFixedString(128);
            }
        ...
        public static void WritePlayerInfo(this ByteWriter writer, PlayerLoadInfo data)
        {
            ...
            if (data.IsBot == false)
            {
                writer.WritePad(64);//why not 32 ??? 
                writer.WritePad(64);//
            }
            else
            {
                writer.WriteFixedString(data.BotName, 130);
                writer.WriteFixedString(data.BotSkinName, 130);
            }
