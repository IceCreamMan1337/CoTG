using CommandLine;
using ChildrenOfTheGraveServerConsole.Properties;
using ChildrenOfTheGrave.ChildrenOfTheGraveServer;
using ChildrenOfTheGrave.ChildrenOfTheGraveServer.Logging;
using ChildrenOfTheGrave.ChildrenOfTheGraveServerConsole.Logic;
using ChildrenOfTheGrave.ChildrenOfTheGraveServerConsole.Utility;
using log4net;
using System;
...
namespace ChildrenOfTheGrave.ChildrenOfTheGraveServerConsole
{
    /// <summary>
    /// Class representing the program piece, or commandline piece of the server; where everything starts (ChildrenOfTheGraveServerConsole -> ChildrenOfTheGraveServer, etc).
    /// </summary>
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            var build = $"ChildrenOfTheGrave Build: 0.0.116";//{BuildInfo.ServerVersion} {BuildInfo.ServerBuildTime}";
            Console.Title = build;
            var text = """
...
#if DEBUG
            // When debugging, optionally the game client can be launched automatically given the path (placed in ChildrenOfTheGraveServerSettings.json) to the folder containing the League executable.
            string config = LoadConfig(parsedArgs.ChildrenOfTheGraveServerSettingsJsonPath, parsedArgs.ChildrenOfTheGraveServerSettingsJson, Encoding.UTF8.GetString(Resources.ChildrenOfTheGraveServerSettings));
            ChildrenOfTheGraveServerConfig configChildrenOfTheGraveServerSettings = ChildrenOfTheGraveServerConfig.LoadFromJson(config);

            if (configChildrenOfTheGraveServerSettings.AutoStartClient)
            {
                LaunchGameClient(configChildrenOfTheGraveServerSettings, parsedArgs);
            }
            else
            {
...
#endif
            // This is where the actual ChildrenOfTheGraveServer starts.
#if !DEBUG
            try
            {
#endif
        ...
        private static void LaunchGameClient(ChildrenOfTheGraveServerConfig ChildrenOfTheGraveServerSettings, ArgsOptions args)
        {
            var leaguePath = ChildrenOfTheGraveServerSettings.ClientLocation;
            ...
            if (File.Exists(leaguePath))
            {
                ...
                _logger.Info("Launching League of Legends. You can disable this in ChildrenOfTheGraveServerSettings.json.");
            ...
            {
                _logger.Warn("Unable to find League of Legends.exe. Check the ChildrenOfTheGraveServerSettings.json settings and your League location.");
            }
        }

        /// <summary>
        /// Used to parse any of the configuration files used for the ChildrenOfTheGraveServer, ex: GameInfo.json or ChildrenOfTheGraveServerSettings.json. 
        /// </summary>
    ...
    /// <summary>
    /// Class housing launch arguments and their parsing used for the ChildrenOfTheGraveServerConsole.
    /// </summary>
    public class ArgsOptions
    {
        ...
        [Option("config-ChildrenOfTheGraveServer", Default = "Settings/ChildrenOfTheGraveServerSettings.json")]
        public string ChildrenOfTheGraveServerSettingsJsonPath { get; set; }

        [Option("config-json", Default = "")]
        public string GameInfoJson { get; set; }

        [Option("config-ChildrenOfTheGraveServer-json", Default = "")]
        public string ChildrenOfTheGraveServerSettingsJson { get; set; }
